...
def to_db(self, table, database_url=None, truncate=False, keys=None, **kwargs):
    method = 'replace' if truncate else 'append'
    with get_db(database_url) as conn, closing(conn.cursor()) as curs:
        write_frame(self, table, conn, flavor='postgresql', if_exists=method)
pd.DataFrame.to_db = to_db

def read_db(q, database_url=None, dsn=None, **kwargs):
    if 'select ' not in q.lower():
        q = 'select * from %s' % q
    with get_db(database_url=database_url) as conn:
        df = read_frame(q, conn, **kwargs)
    return df
...
def write_frame(frame, name, con, flavor='sqlite', if_exists='fail', keys=None, **kwargs):
    """
    ...
    if exists and if_exists == 'replace':
        if flavor == 'vertica': 
            create = "TRUNCATE TABLE %s" % name
        else:
            create = "TRUNCATE %s" % name
    elif not exists:
        create = get_schema(frame, name, flavor, keys=keys)
    ...
    safe_names = [s.replace(' ', '_').replace('+ ', '').strip().lower() for s in frame.columns]
    #safe_names = [slugify(s, separator='_') for s in frame.columns]
    flavor_picker = {'sqlite' : _write_sqlite,
                     'mysql' : _write_mysql,
                     'postgresql' : _write_postgresql,
                     'vertica' : _write_vertica}
...
def _write_vertica(frame, table, names, cur):
    columns = ','.join(list(names))
    with NamedTemporaryFile(delete=True) as f, closing(StringIO()) as s:
        frame.to_csv(s, index=False, header=False, quoting=csv.QUOTE_ALL, na_rep="\\N", encoding='utf-8', sep='|')
        s.seek(0)
        f.write(s.getvalue().replace('\"\\N\"', ''))
        f.flush()
        sql = "COPY {table} ({columns}) FROM LOCAL '{file_name}' WITH DELIMITER '|' ENCLOSED BY '\"' NULL '' REJECTED DATA  '/data/rejected.log' EXCEPTIONS '/data/exceptions.log' ".format(table=table, columns=columns, file_name=f.name) #ABORT ON ERROR
        cur.execute(sql)
        cur.commit()
...
def _write_postgresql(frame, table, names, cur):
    ...
    with closing(StringIO()) as s:
        frame.to_csv(s, index=False, header=False, quoting=csv.QUOTE_ALL, na_rep='\\N', encoding='utf-8')
        s.seek(0) #Return to beginning of file to read the contents
        ## Pandas exports the null value without quotes, so we replace it with quotes so we replace it with a blank string
        ## The alternative is to use QUOTE_MINIMAL and then all fields with a comma will contain unnecessary
        cur.copy_expert("COPY {table} ({columns}) FROM STDIN WITH CSV".format(table=table, columns=columns), StringIO(s.getvalue().replace('"\\N"', '')))
...
def table_exists(name, con, flavor):
    flavor_map = {
        ...
        'postgresql' : "select relname from pg_stat_user_tables WHERE schemaname='public' and relname='%s'" % name,
        'vertica' : "SELECT * FROM v_catalog.tables WHERE table_schema='public' AND table_name='%s'" % name }
...
def get_sqltype(pytype, flavor):
    sqltype = {'mysql': 'VARCHAR (63)',
               'sqlite': 'TEXT',
               'postgresql' : 'TEXT',
               'vertica' : 'VARCHAR (1000)' }

    if issubclass(pytype, np.floating):
        sqltype['mysql'] = 'FLOAT'
        sqltype['sqlite'] = 'REAL'
        sqltype['postgresql'] = 'FLOAT'
        sqltype['vertica'] = 'FLOAT'
    ...
    if issubclass(pytype, np.bool_):
        sqltype['sqlite'] = 'INTEGER'
        sqltype['postgresql'] = 'BOOL'
        sqltype['vertica'] = 'BOOL'
...
def get_schema(frame, name, flavor, keys=None):
    ...
    # Replace spaces in DataFrame column names with _.
    #safe_columns = [slugify(s, separator='_') for s in frame.dtypes.index]
